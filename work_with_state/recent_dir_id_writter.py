...
def set_id_recent_dirs(dir_id: str, clear: bool = False) -> None:
    if clear:
        with open('./state/recent_dirs_id', 'w') as file_recent_dir:
            file_recent_dir.write(dir_id)
        return

    with open('./state/recent_dirs_id', 'a') as file_recent_dir:
        file_recent_dir.write(dir_id + '\n')
...
def get_id_recent_dir() -> str:
    with open('./state/recent_dirs_id', 'r') as file_recent_dir:
